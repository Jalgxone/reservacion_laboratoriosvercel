<?php
$title = "Crear Reserva | Sistema de Reservación";
    ...
    <div style="padding: var(--espacio-md);">
        <?php if (!empty($errors)): ?>
            <div class="badge badge-error" style="width: 100%; margin-bottom: var(--espacio-md); padding: var(--espacio-sm);">
                <?php
                // $errors puede ser lista de strings o array asociativo campo => [mensajes]
        ...
        <form method="post" action="<?= $_SERVER['SCRIPT_NAME'] ?>?url=reservas/store">
            <div class="grid" style="grid-template-columns: 1fr 1fr; gap: var(--espacio-md);">
                ...
                <div class="form-group">
                    <label for="id_estado" class="form-label">Estado Inicial</label>
                    <select name="id_estado" id="id_estado" class="form-control">
                            <?php if (!empty($estados)): ?>
                                <?php foreach ($estados as $es): ?>
                                    <option value="<?= $es['id_estado'] ?>" <?= (!empty($old['id_estado']) && $old['id_estado'] == $es['id_estado']) ? 'selected' : '' ?>><?= htmlspecialchars($es['nombre_estado']) ?></option>
                                <?php endforeach; ?>
                            <?php else: ?>
            ...
            <div class="grid" style="grid-template-columns: 1fr 1fr; gap: var(--espacio-md);">
                <div class="form-group">
                    ...
                    <input type="datetime-local" id="fecha_inicio" name="fecha_inicio" class="form-control" value="<?= !empty($old['fecha_inicio']) ? str_replace(' ', 'T', $old['fecha_inicio']) : '' ?>">
                </div>
                
                <div class="form-group">
                    <label for="fecha_fin" class="form-label">Fecha y Hora de Fin</label>
                    <input type="datetime-local" id="fecha_fin" name="fecha_fin" class="form-control" value="<?= !empty($old['fecha_fin']) ? str_replace(' ', 'T', $old['fecha_fin']) : '' ?>">
                </div>
            </div>
            
            <div class="form-group">
                <label for="motivo_uso" class="form-label">Motivo del Uso</label>
                <input type="text" id="motivo_uso" name="motivo_uso" class="form-control" value="<?= htmlspecialchars($old['motivo_uso'] ?? '') ?>" placeholder="Ej. Práctica de Redes, Examen, etc.">
            </div>
