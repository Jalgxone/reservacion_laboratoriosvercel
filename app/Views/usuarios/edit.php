...
<div class="pagina-cabecera">
    <div>
        <h1 class="pagina-titulo">Editar Usuario</h1>
        <p class="pagina-subtitulo">Modificar los datos del usuario</p>
    </div>
    <div class="acciones">
...
<div class="card">
    <form action="<?= $appRoot ?>/usuarios/update/<?= $user['id_usuario'] ?>" method="post" novalidate>
        <div class="card-body" style="padding: var(--espacio-xl) var(--espacio-lg);">
            ...
            <div class="grid" style="grid-template-columns: 1fr 1fr; gap: var(--espacio-lg); margin-bottom: var(--espacio-xl);">
                <div class="form-group">
                    ...
                    <div style="position: relative;">
                        <input type="text" name="nombre_completo" class="form-control" style="padding-left: 3rem;" value="<?= htmlspecialchars($user['nombre_completo'] ?? '') ?>" required>
                ...
                <div class="form-group">
                    ...
                    <div style="position: relative;">
                        <input type="text" name="apellido" class="form-control" style="padding-left: 3rem;" value="<?= htmlspecialchars($user['apellido'] ?? '') ?>" required>
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="position: absolute; left: 1rem; top: 50%; transform: translateY(-50%); color: var(--color-texto-claro);"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>
                    </div>
            ...
            <div class="form-group" style="margin-bottom: var(--espacio-lg);">
                ...
                <div style="position: relative;">
                    <input type="email" name="email" class="form-control" style="padding-left: 3rem; background-color: var(--color-fondo-claro); cursor: not-allowed;" value="<?= htmlspecialchars($user['email'] ?? '') ?>" readonly>
            ...
            <div class="grid" style="grid-template-columns: 1fr 1fr; gap: var(--espacio-lg); margin-bottom: var(--espacio-xl);">
                <div class="form-group">
                    ...
                    <div style="position: relative;">
                        <input type="text" name="cedula_identidad" class="form-control" style="padding-left: 3rem; background-color: var(--color-fondo-claro); cursor: not-allowed;" value="<?= htmlspecialchars($user['cedula_identidad'] ?? $user['cedula'] ?? '') ?>" placeholder="V-12345678" readonly>
                ...
                <div class="form-group">
                    ...
                    <div style="position: relative;">
                        <input type="text" name="telefono" class="form-control" style="padding-left: 3rem;" value="<?= htmlspecialchars($user['telefono'] ?? '') ?>" placeholder="+584121234567" required>
            ...
            <div class="grid" style="grid-template-columns: 1fr 1fr; gap: var(--espacio-lg); margin-bottom: var(--espacio-xl);">
                <div class="form-group">
                    ...
                    <div style="position: relative;">
                        <select name="id_rol" id="id_rol" class="form-control" style="padding-left: 3rem; appearance: none;">
                            <?php foreach ($roles as $r): ?>
                                <option value="<?= $r['id_rol'] ?>" <?= (isset($user['id_rol']) && $user['id_rol'] == $r['id_rol']) ? 'selected' : '' ?>><?= htmlspecialchars($r['nombre_rol']) ?></option>
                            <?php endforeach; ?>
                        </select>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (rolSelect) {
        let previousValue = rolSelect.value;
        
        rolSelect.addEventListener('change', function(e) {
            window.appConfirm('¿Estás seguro de que deseas cambiar el rol de este usuario?').then(function(confirmed) {
                if (!confirmed) {
                    rolSelect.value = previousValue;
                } else {
                    previousValue = rolSelect.value;
                }
            });
        });
    }
