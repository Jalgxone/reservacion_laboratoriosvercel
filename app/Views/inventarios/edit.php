...
<div class="pagina-cabecera">
    <div>
        <h1 class="pagina-titulo">Editar Equipo</h1>
        <p class="pagina-subtitulo">Actualizar información del ítem #<?= htmlspecialchars($item['id_equipo']) ?></p>
    </div>
</div>
...
<div class="card" style="max-width: 600px; margin: 0 auto;">
    ...
    <div style="padding: var(--espacio-md);">
        ...
        <form method="post" action="<?= $_SERVER['SCRIPT_NAME'] ?>?url=inventarios/update/<?= $item['id_equipo'] ?>" novalidate>
            ...
            <div class="form-group">
                <label for="estado_operativo" class="form-label">Estado de Operación</label>
                <select name="estado_operativo" id="estado_operativo" class="form-control">
                    <option <?= (isset($old['estado_operativo']) ? ($old['estado_operativo'] == 'Operativo') : ($item['estado_operativo']=='Operativo')) ? 'selected' : '' ?>>Operativo</option>
                    <option <?= (isset($old['estado_operativo']) ? ($old['estado_operativo'] == 'En Reparación') : ($item['estado_operativo']=='En Reparación')) ? 'selected' : '' ?>>En Reparación</option>
                    <option <?= (isset($old['estado_operativo']) ? ($old['estado_operativo'] == 'Baja') : ($item['estado_operativo']=='Baja')) ? 'selected' : '' ?>>Baja</option>
                </select>
            </div>
